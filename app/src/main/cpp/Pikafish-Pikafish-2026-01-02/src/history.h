...
// MultiArray is a multi-dimensional array based on std::array
...
struct MultiArray<T, Size> : public std::array<T, Size> {
    ...
    void fill(const U& v) {
        for (auto& entry : *this)
        {
            if constexpr (requires { entry.fill(v); })
                entry.fill(v);
            else
                entry = v;
        }
...
enum CorrHistType {
    ...
    Minor,         // By color and positions of minor pieces (Knight, Cannon)
