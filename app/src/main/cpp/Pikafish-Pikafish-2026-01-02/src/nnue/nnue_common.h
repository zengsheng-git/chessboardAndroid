...
// Forward declaration of read_compressed_nnue
std::string read_compressed_nnue(const std::string& filename);

// Utility to combine hashes
template<typename T>
...
// Fixed-size string that is trivially copyable
...
struct FixedString {
    ...
    FixedString(std::string_view sv) {
        std::copy_n(sv.data(), std::min(sv.size(), N - 1), data);
    }
...
inline std::size_t get_raw_data_hash(const std::array<T, N>& data) {
    std::size_t h = 0;
...
template<typename T, std::size_t N>
inline std::size_t get_raw_data_hash(const T (&data)[N]) {
    std::size_t h = 0;
    for (const auto& v : data)
        hash_combine(h, v);
    return h;
...
template<std::size_t N>
struct hash<Stockfish::FixedString<N>> {
    std::size_t operator()(const Stockfish::FixedString<N>& fs) const noexcept {
        return std::hash<std::string_view>{}(static_cast<std::string_view>(fs));
    }
};
