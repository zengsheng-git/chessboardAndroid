...
#if !defined(NO_PREFETCH) && (defined(__GNUC__) || defined(__clang__))
    #define prefetch(addr) __builtin_prefetch((addr))
#elif !defined(NO_PREFETCH) && defined(_MSC_VER)
...
inline uint64_t mul_hi64(uint64_t a, uint64_t b) {
    ...
    uint64_t mid2 = a_hi * b_lo;
    uint64_t lo_lo = a_lo * b_lo;
    uint64_t hi_hi = a_hi * b_hi;
    uint64_t mid = mid1 + mid2 + (lo_lo >> 32);
    return hi_hi + (mid >> 32) + ((mid1 & 0xFFFFFFFF) + (mid2 & 0xFFFFFFFF) + (lo_lo >> 32) >= 0x100000000 ? 1 : 0); // simplified, needs care
}
#endif
...
struct PRNG {
    ...
    T rand() {
        ...
        s ^= s << 25;
        s ^= s >> 27;
        return T(s * 2685821657736338717LL);
    }
