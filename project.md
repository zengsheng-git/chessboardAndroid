...
4. 丢王修复带一致性约束：王以外部分恰好是一步干净棋才补回；王位记忆
   ...
   **修复帧（`repaired`）与基线存在差异时同样按噪声跳过**——高亮遮挡期内
   识别抖动的不只是王，修复帧上的"一步棋"可能是抖动假象，不得据此翻轮次；
...
| 遮挡期后提示轮次错且重复旧招（0.0.35） | 修复帧上的识别抖动（马在两格间闪）被当成红方又走一步，翻错轮次并重析旧局面 | `Frame.repaired` 标记：修复帧与基线有差异时按噪声跳过，不翻轮次、不分析、不锚定；另修正走子日志轮次文案反了的问题 |
...
可以上架，但存在三层限制：**GPL 开源合规**（Pikafish 与识别模型均为 GPLv3 派生，
上架即须整体开源）、**功能定位风险**（"实时对局辅助"在 Google Play 与国内商店
均有拒审/下架风险，建议定位调整为"复盘/训练工具"）、**资质与技术要求**
（软著、App 备案、隐私政策、targetSdk 35+ 等）。

详细分析、各商店要求清单与建议路线见 **[docs/publishing.md](docs/publishing.md)**。
维持 GitHub/网盘分发 APK 则不受上述限制。
